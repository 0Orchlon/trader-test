...
# PERSONAL-3 — Ажлын задаргаа (T-01…T-52)

**Ажил:** PERSONAL-3 · **Салбар:** `issue/personal-3` · **Шат:** PLAN_TASK
**Эрх бүхий оролт:** `spec.md` v1.1 (AC-1…AC-38) · **Хамт:** `plan.md` v1.1 (M1…M6, P-1…P-8)

> **v1.1 засвар:** T-01…T-41 **өөрчлөгдөөгүй** (хамрах хүрээ, DoD, дугаарлал
> хэвээр — Hefesto-д бүртгэгдсэн ID-ууд хүчинтэй). Хяналтаас ирсэн OP-11…OP-13
> нь **T-44…T-52** есөн шинэ task болж §10-д нэмэгдэв; тэдгээр нь одоо байгаа
> эпик A, B, C, D, G, H-д харьяалагдана. T-42, T-43 нь backlog-д аль хэдийн
> эзлэгдсэн дугаар тул T-44-өөс үргэлжлүүлэв.
...
- Pydantic Settings: Alpaca endpoint (paper анхдагч), `LIVE_TRADING_ENABLED`
  (анхдагч `false`), §7-ийн 10 хязгаарын нэр — **анхдагч утгагүй, дутуу бол
  startup унана**.
...
### T-41 · Runbook
**Хэмжээ:** S · **Хамаарал:** T-16, T-25, T-52 · **AC:** AC-15, AC-18, AC-37
- Унтраах бэлтгэл → унтраалт → дахин асаалт → `activate` хийх ил алхмууд
  (AC-37 — хадгалагдсан төлөвөөс `ACTIVE` руу буцах цорын ганц зам).
...
## 10. v1.1-ийн нэмэлт task (T-44…T-52)

Эх шаардлага: `spec.md` v1.1 §2-ийн **OP-11** (хэн юунд арилжаа хийж байна),
**OP-12** (гарын арилжаа), **OP-13** (унтраах бэлтгэл → зогсолт → идэвхжүүлэлт).

### T-44 · Schema өргөтгөл: `orders.origin` + `system_state` (эпик A, M1)
**Хэмжээ:** S · **Хамаарал:** T-02 · **AC:** AC-29, AC-37
- Alembic migration: `orders.origin` (`NOT NULL`, enum `research_agent` |
  `auto_tuning` | `manual_operator` | `external`) + `origin_detail` (provider/model
  эсвэл actor). Байгаа мөр байхгүй тул backfill шаардлагагүй; anхдагч утга
  ТАВИХГҮЙ — дуудагч бүр ил зааж өгнө.
- `system_state` хүснэгт: `state` (`active` | `winding_down` | `halted`),
  `reason`, `changed_by`, `changed_at` (`timestamptz`), `wind_down_deadline`.
  Түүх нь append-only — сүүлийн мөр нь одоогийн төлөв.
- `WIND_DOWN_GRACE` config-д нэмэгдэнэ (§7-ийн 10 дахь хязгаар; анхдагч утгагүй).
  Grace нь **хуанлийн цагаар** тоологдоно (`plan.md` OQ-8-ийн таамаг) — энэ
  сонголт код дотор тайлбартай бичигдэнэ.
- **DoD:** (а) `origin`-гүй order INSERT хийх оролдлого DB дээр унана;
  (б) enum-аас гадуур утга татгалзана; (в) `system_state`-ийн мөрийг UPDATE /
  DELETE хийх оролдлого татгалзана (append-only); (г) `WIND_DOWN_GRACE`
  тохируулаагүй бол app эхлэхгүй (T-03-ийн зарчимтай ижил).

### T-45 · Risk Agent-ийн wind-down дүрэм (эпик C, M2)
**Хэмжээ:** M · **Хамаарал:** T-11, T-44 · **AC:** AC-35, AC-36
- Risk-ийн оролтод системийн төлөв нэмэгдэнэ. `winding_down` үед: exposure
  **нэмэгдүүлэх** бүх order `REJECT(reason="winding_down")`; зөвхөн багасгах /
  хаах order цааш шалгагдана. `halted` үед бүх order `REJECT`.
- §7-ийн хатуу хязгаарууд wind-down үед СУЛРАХГҮЙ — хаах order ч хязгаараа
  дайрна (fat-finger, `RESTRICTED_SYMBOLS` г.м.).
- «Exposure нэмэгдүүлэх» тодорхойлолт: шинэ symbol, байгаа позицийн чиглэлд
  нэмэх, эсрэг тийш позицийг хаагаад хэтрүүлэн эргүүлэх (хэсэгчилсэн хаалт
  зөвшөөрөгдөнө, эргүүлэлт зөвшөөрөгдөхгүй).
- **DoD:** (а) Hypothesis ≥10 000 кейс: `winding_down` төлөвт нэмэгдүүлэх
  `APPROVE` = 0; (б) хаах order дамждаг тусдаа кейс; (в) эргүүлэх order
  (хаалт + эсрэг нээлт) татгалзана; (г) `halted` төлөвт `APPROVE` = 0;
  (д) mutation testing — эдгээр шалгалтыг эвдэхэд тест унана (T-12-ийн gate-д
  нийлнэ, Risk модулийн 100% хучилт хэвээр).

### T-46 · Origin бүртгэл + attribution read API (эпик B, M1)
**Хэмжээ:** M · **Хамаарал:** T-07, T-44 · **AC:** AC-29, AC-30
- Order үүсгэх зам бүр `origin` + `origin_detail`-ыг ил дамжуулна (анхдагч байхгүй).
- Position-ийн origin нь `client_order_id`-аар локал `orders`-той тааруулж
  тогтоогдоно. Таарахгүй бол `external` — ХЭЗЭЭ Ч таамаглахгүй, хамгийн ойрын
  order-т «наахгүй».
- `GET /attribution`: origin бүрээр нээлттэй позицтой symbol, нээлттэй order-той
  symbol, сүүлийн шийдвэрийн UTC хугацаа. Гаралт нь `orders`/`positions`-ийн
  бодит мөрөөс угсарна; тусдаа тооцоолсон кэш БАЙХГҮЙ.
- Хариу нь `source` талбартай хэвээр (AC-20) — attribution нь горимыг бүрхэхгүй.
- **DoD:** (а) Alpaca-д гараар нээсэн позиц `external` гэж гарахыг батлах
  integration тест; (б) `GET /attribution`-ийн symbol олонлог нь DB-ийн
  нээлттэй позиц/order-ийн symbol олонлогтой ЯГ тэнцүү (set харьцуулалт);
  (в) бодит мөргүй symbol гаралтад орвол тест унана.

### T-47 · Гарын order endpoint (эпик C, M2)
**Хэмжээ:** M · **Хамаарал:** T-13, T-45 · **AC:** AC-31, AC-32, AC-33
- `POST /api/v1/orders/manual` → Risk Agent → Execution. **Шинэ submit зам
  ҮҮСГЭХГҮЙ**: T-13-ийн байгаа орох цэгийг дуудна; T-14-ийн статик хаалт
  өөрчлөгдөхгүй (allowlist өргөсөхгүй).
- `origin = manual_operator`, actor ба UI-ийн баталгаажуулалтын тэмдэг
  `audit_log`-д бичигдэнэ. `agent_decisions`-д мөр ҮҮСГЭХГҮЙ (энэ нь agent-ийн
  шийдвэр биш) — grounding checker хамаарахгүй.
- Risk `ESCALATE_TO_HUMAN` бол approval queue-д мөр үүсгэхийн оронд хүсэлт
  `confirmation_token`-гүйгээр татгалзаж, UI дахин баталгаажуулалт асууна
  (спек A-8).
- Төлөв `halted` бол 409/татгалзал; `winding_down` бол зөвхөн багасгах order.
- **DoD:** (а) Risk REJECT үед Alpaca mock дуудалт = 0; (б) `halted` төлөвт
  гарын order татгалзаж, дуудалт = 0; (в) `winding_down` үед нэмэгдүүлэх
  гарын order татгалзана, хаах order дамжина; (г) T-14-ийн статик тест
  өөрчлөлтгүйгээр ногоон хэвээр (шинэ дуудагч package нэмэгдээгүй);
  (д) `audit_log`-д гарын order-ийн мөр actor-той бүртгэгдсэн.

### T-48 · Системийн төлөвийг agent-т хүргэх (эпик D, M3)
**Хэмжээ:** S · **Хамаарал:** T-18, T-44 · **AC:** AC-34
- Tool Contract v1-ийн tool result бүрд `system_state` талбар нэмэгдэнэ
  (`active` | `winding_down` + үлдсэн секунд | `halted`). Schema-д `required`.
- `winding_down` үед `propose_order`-ийн handler нь эрсдэл нэмэгдүүлэх саналыг
  Risk-д хүрэхээс өмнө ил татгалзаж, шалтгааныг agent-т буцаана (Risk-ийн
  T-45 хаалт нь хоёр дахь давхарга хэвээр — энэ нь түүнийг ОРЛОХГҮЙ).
- **DoD:** (а) төлөв солигдсоны дараах эхний tool result-д шинэ утга гарна;
  (б) `system_state`-гүй tool result schema дээр унана; (в) handler-ийн
  татгалзал нь Risk-ийн хаалтыг сулруулаагүйг батлах тест (T-45-ийн suite
  өөрчлөлтгүй ногоон).

### T-49 · Гарын арилжааны ticket UI (эпик G, M2)
**Хэмжээ:** M · **Хамаарал:** T-29, T-47 · **AC:** AC-31, AC-33
- Ticket маягт: symbol, тал, тоо ширхэг, order төрөл, үнэ. Илгээхийн өмнө
  тооцоолсон notional ба Risk-ийн урьдчилсан үнэлгээ харагдана.
- Босгоос дээш бол ил баталгаажуулалтын алхам (token урсгал).
- `halted` үед товч идэвхгүй + шалтгаан харагдана; `winding_down` үед зөвхөн
  позиц багасгах чиглэл сонгох боломжтой.
- One-click / chart-аас чирж order тавих БАЙХГҮЙ (`plan.md` P-8); спек §8-ийн
  dark-pattern хориг хүчинтэй.
- **DoD:** (а) Playwright: `halted` төлөвт илгээх товч идэвхгүй; (б) босгоос
  дээш order баталгаажуулалтгүйгээр илгээгдэхгүй; (в) `winding_down` үед
  нэмэгдүүлэх чиглэл сонгох боломжгүй.

### T-50 · «Хэн юунд арилжаа хийж байна» харагдац (эпик G, M2)
**Хэмжээ:** M · **Хамаарал:** T-29, T-46 · **AC:** AC-29, AC-30
- `GET /attribution`-ийг харуулах дэлгэц: origin (agent + provider/model,
  auto-tuning, manual, external) тус бүрээр symbol, позиц, нээлттэй order,
  сүүлийн шийдвэрийн хугацаа.
- Position ба order хүснэгтийн мөр бүрд origin багана нэмэгдэнэ.
- Мөрөөс шийдвэрийн feed (T-32) руу шилжих холбоос — «яагаад» руу нэг даралт.
- **DoD:** (а) Playwright: мөр бүр origin шошготой; (б) `external` позиц нь
  agent-ийн нэрээр ХЭЗЭЭ Ч харагдахгүй; (в) харагдацын symbol жагсаалт нь
  API-ийн буцаасантай тэнцүү.

### T-51 · Системийн төлөвийн самбар (эпик G, M2)
**Хэмжээ:** M · **Хамаарал:** T-29, T-15, T-44 · **AC:** AC-34, AC-36, AC-38
- Бүх дэлгэц дээр байнгын төлөвийн заалт: `ACTIVE` | `WINDING_DOWN` + үлдсэн
  хугацаа | `HALTED` + шалтгаан.
- Гурван товч, утга нь ялгаатай: **Зогсоо** (`POST /kill-switch` — тэр дор нь),
  **Унтраах бэлтгэл** (`POST /system/wind-down` — эхлээд хаах цонх, дараа нь
  зогсох), **Идэвхжүүл** (`POST /system/activate` — ил баталгаажуулалттай).
- `Идэвхжүүл` нь зөвхөн `HALTED`/`WINDING_DOWN` үед идэвхтэй. Breaker-ийн
  шалтгаан цэвэрлэгдээгүй бол шалтгаан ба runbook-ийн заавар харагдана.
- **DoD:** (а) Playwright: дэлгэц бүр дээр заалт байна; (б) `ACTIVE` үед
  `Идэвхжүүл` идэвхгүй; (в) wind-down дарснаас хойш 1 секунд дотор бүх нээлттэй
  таб дээр төлөв солигдсоныг WS-ээр батлах тест; (г) хоёр товчийг андуурахааргүй
  ялгаатай (текст + баталгаажуулалтын өөр өөр асуулт).

### T-52 · Төлөвийн машины E2E: wind-down → restart → activate (эпик H, M2)
**Хэмжээ:** M · **Хамаарал:** T-45, T-47, T-51 · **AC:** AC-33, AC-36, AC-37
- Бүтэн мөчлөг: `ACTIVE` → wind-down → нэмэгдүүлэх order татгалзана / хаах
  order дамжина → grace дуусна (цаг хөлдөөх) → `HALTED` → **процессыг унагаж
  дахин асаана** → төлөв `HALTED` хэвээр, шинэ order = 0 → баталгаажуулалтгүй
  `activate` татгалзана → баталгаажуулалттай `activate` → `ACTIVE`.
- Breaker-ийн шалтгаан цэвэрлээгүй үеийн `activate` татгалзана (T-16-тэй холбоно).
- T-38-ийн chaos suite-д нэмэлт кейс: wind-down дунд Redis унах — төлөв
  Postgres-ээс сэргэнэ (`plan.md` R-10).
- **DoD:** (а) дахин асаалтын дараа `ACTIVE` болбол тест унана; (б) grace
  дууссаны дараа позицийн тоо, `qty` өөрчлөгдөөгүй (автомат liquidation алга);
  (в) Redis унасан үед төлөв алдагдахгүй; (г) suite нь UAT_TEST-ийн заавал
  гүйцэтгэх хэсэг.
...
| T-53 | Wind-down-ийн grace дуусахад үлдсэн позицыг автоматаар хаах | OQ-7-ийн хариу «хаах» бол — спек §9 A-7 ба AC-36 дахин бичигдэнэ (`plan.md` R-8) |
| T-54 | `HALTED` үед гарын яаралтай гарц (зөвхөн хаах order) | OQ-6-ийн хариу «хэрэгтэй» бол — спек §9 A-9 ба AC-33 дахин бичигдэнэ |
| T-55 | Стратеги / symbol тус бүрийн зогсоолт | `plan.md` P-6 — шинэ спек шаардана |
...
| AC-28 | T-01, T-40 |
| AC-29 | T-44, T-46, T-50 |
| AC-30 | T-46, T-50 |
| AC-31 | T-47, T-49 |
| AC-32 | T-47 |
| AC-33 | T-47, T-49, T-52 |
| AC-34 | T-48, T-51 |
| AC-35 | T-45 |
| AC-36 | T-45, T-51, T-52 |
| AC-37 | T-44, T-52, T-41 |
| AC-38 | T-51 |

**Task-гүй AC:** байхгүй (38/38). **AC-гүй task:** байхгүй (50/50 — T-01…T-41,
T-44…T-52; T-42, T-43 нь backlog).
